...
try {
  if (Csv1) {
    ...
    const Array1 = Csv1.toString().split('\r');
    var string1 = Array1[0].split(',');
    for (let i = 1; i < Array1.length; i++) {
      string1.push(Array1[i].split(','));
    }
  } else {
    throw new Error('error in importing matches.csv file');
  }
} catch (err) {
  console.log(err);
}
try {
  if (Csv2) {
    ...
    const Array2 = Csv2.toString().split('\r');
    var string2 = Array2[0].split(',');
    for (let i = 1; i < Array2.length; i++) {
      string2.push(Array2[i].split(','));
    }
  } else {
    throw new Error('error in importing deliveries.csv file');
  }
} catch (err) {
  console.log(err);
}
try {
  ...
  if (PerYearMatchesFunction) {
    ...
    throw new Error('error peryearmatches file not found');
  }
} catch (err) {
  console.log(err);
}
try {
  ...
  if (PerYearWinnerFunction) {
    ...
    throw new Error('error peryearwiner file not found');
  }
} catch (err) {
  console.log(err);
}
try {
  ...
  if (ExtraRunFunction) {
    ...
    throw new Error('error extrarun file not found');
  }
} catch (err) {
  console.log(err);
}
try {
  ...
  if (Top10EconomicBowlerFunction) {
    ...
    throw new Error('top10economicbowler file not found');
  }
} catch (err) {
  console.log(err);
}
...
Promise.all(
  PerYearMatchesFunction(string1, 4),
  PerYearWinnerFunction(string1, 4),
  ExtraRunFunction(string2, 4),
  Top10EconomicBowlerFunction(string2, 4)
)
  .then((results) => {
    console.log('All function run succesfully');
  })
  .catch((error) => {
    console.log('Failed! All function not run succesfully');
  });
